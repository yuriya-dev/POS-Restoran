    ...
    return (
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4">
                {error && <div className="bg-red-50 text-red-600 p-3 text-sm rounded border border-red-200">{error}</div>}
                
                {/* Input Nama Meja */}
                <div>
                    <label className="block text-sm font-medium text-gray-700">Nama/Nomor Meja</label>
                    <input
                        name="number"
                        type="text"
                ...
                <div>
                    ...
                    <div className="relative">
                        <select
                            ...
                            value={formData.type}
                            onChange={handleChange}
                            className="block w-full pl-4 pr-10 py-2.5 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white appearance-none cursor-pointer transition-all hover:border-gray-400"
                        >
                            {TABLE_TYPES.map(t => (
...
const TableCard = ({ table, onEdit, onToggleStatus, onDelete }) => {
    ...
    return (
        <div className={`p-4 rounded-xl shadow-sm border-2 transition-all duration-200 flex flex-col justify-between bg-white h-full relative group
            ${isOccupied ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-blue-400 hover:shadow-md'}`}>
            ...
            <p className="text-sm text-gray-500 mb-4 flex items-center">
                ...
                Kapasitas: <span className="font-semibold text-gray-700 ml-1">{table.capacity}</span>
            </p>

            <div className="flex justify-between space-x-2 border-t pt-3 mt-auto">
...
const TableManagement = () => {
    ...
    const serviceTableColumns = [
        ...
        { header: 'Tipe', render: () => <span className="text-indigo-600 font-medium">Layanan</span> },
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <p className="text-gray-500">Atur layout meja dine-in dan titik layanan.</p>
