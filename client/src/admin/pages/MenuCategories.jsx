    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? `Edit Kategori` : 'Tambah Kategori'}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Kategori</label>
                    <input name="name" type="text" value={formData.name} onChange={handleChange} className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg p-2.5 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-400 transition-colors" required />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Ikon</label>
                    <div className="flex items-center space-x-3 mt-1">
                        <div className="p-2 bg-gray-100 dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600"><SelectedIcon className="w-6 h-6 text-blue-600 dark:text-blue-400" /></div>
                        <div className="relative grow">
                            <select name="icon" value={formData.icon} onChange={handleChange} className="block w-full pl-3 pr-10 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white appearance-none">
                ...
                <div>
                    ...
                    <input name="sort_order" type="number" value={formData.sort_order} onChange={handleChange} className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg p-2.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-white" required />
                </div>
...
const MenuCategories = () => {
    ...
    const columns = [
        ...
        { header: 'Urutan', accessor: 'sort_order', render: (val) => <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 text-xs font-medium px-2.5 py-0.5 rounded transition-colors">{val}</span> },
        { header: 'Jumlah Menu', accessor: 'categoryId', render: (id) => {
            const count = categoryItemCounts[id] || 0;
            return <span className={count > 0 ? 'text-green-600 dark:text-green-400 font-bold' : 'text-gray-400 dark:text-gray-500'}>{count} menu</span>;
        }},
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 space-y-6 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-200">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center"><Tag className="w-6 h-6 mr-2 text-blue-600 dark:text-blue-400" /> Kategori Menu</h1>
            ...
            <Card className="p-0 overflow-hidden dark:bg-gray-800 dark:border-gray-700 transition-colors">
